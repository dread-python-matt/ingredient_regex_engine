...
class TestRegexRegistryInit:
    def test_init_happy_path(self):
        entries = [_entry("mleko", ["mleko"]), _entry("masło", ["masło"])]
    ...
    def test_init_duplicate_entries(self):
        entries = [_entry("mleko", ["mleko"]), _entry("mleko", ["masło"])]
...
class TestRegexRegistryAddVariant:
    ...
    def test_add_variant_happy_path(self, initial_stem, initial_variants, new_variant):
        r = RegexRegistryDefault(
            RegexKind.INGREDIENT_NAME, [_entry(initial_stem, initial_variants)]
        )
...
def entries():
    ...
    vanilla_pudding = _entry(
        "budyń_waniliowy", ["budyń waniliowy", "budynie waniliowe", "budyniu waniliowego"]
    )
...
class TestRegexRegistryMatchBest:
    ...
    @pytest.mark.parametrize("text", ["mleczny koktail", "kakaowy", "5 budyni", "masło"])
...
class TestRegexRegistrySwapMatch:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "5 łyżek budyniu wanilinowego i 2 łyżki budyniu waniliowego",
                "5 łyżek SWAP wanilinowego i 2 łyżki SWAP",
            ),
